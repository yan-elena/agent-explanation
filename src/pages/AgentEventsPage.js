...
function AgentEventsPage(props) {
    ...
    const filterEvents = (event)  => {
        ...
        setFilteredEvents(props.agent.log.filter(log => (log.message.type.replace(/\B([A-Z])\B/g, ' $1') + " " + log.message.log).toLowerCase().includes(filter)))
    }

    return (
        <div>
            <h2 className="m-4">{props.agent ? "Agent " + props.agent.name : ""}</h2>
            <ControlForm filterEvents={filterEvents}/>
