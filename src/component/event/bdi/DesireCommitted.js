...
function DesireCommitted(props) {
    ...
    if (im[0]) {
        if (im[0].trigger !== desire) {
            parentDesire = ["Intention " + desire + "/" + id + " is an intention created from intention " + im[0].trigger + "/" + id,
                <br/>]
        } else {
            parentDesire = im[1] ? ["Intention " + desire + "/" + id + " is an intention created from intention " + im[1].trigger + "/" + id, <br/>] : []
        }
    }
